...
const VITE_API_BASE_URL = "https://ai-expense-tracker-ckpm.onrender.com/api"; 
// const VITE_API_BASE_URL = "http://localhost:5000/api"; 

const createExpense = async (formData) => {
  try {
    const response = await axios.post(`${VITE_API_BASE_URL}/expenses/create-expense`, formData);
...
const setFinancialInfo = async (formData) => {
  try {
    const response = await axios.post(`${VITE_API_BASE_URL}/expenses/set-financial-info`, formData);
    return {data:response.data.expense, message: response.data.message};
  } catch (error) {
...
const getExpenseList = async () => {
  try {
    const response = await axios.get(`${VITE_API_BASE_URL}/expenses/get-expense`);
    return {data:response.data.expense, message: response.data.message};
  } catch (error) {
...
const getFinancialInfo = async () => {
  try {
    const response = await axios.get(`${VITE_API_BASE_URL}/expenses/get-financial-info`);
...
const fetchChatInsight = async (requestBody) => {
  try {
    ...
    const response = await axios.post(`${VITE_API_BASE_URL}/chat`, requestBody, {
      headers: {
